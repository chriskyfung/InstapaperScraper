...
def test_save_to_csv_without_read_url(sample_articles, output_dir):
    """Test saving articles to a CSV file without the read URL."""
    csv_file = output_dir / "bookmarks.csv"
    articles = sample_articles()
    save_to_csv(articles, str(csv_file), add_read_url=False)
...
def test_save_to_csv_with_read_url(sample_articles, output_dir):
    """Test saving articles to a CSV file with a read URL."""
    csv_file = output_dir / "bookmarks_with_prefix.csv"
    articles = sample_articles()
    for article in articles:
        article["read_url"] = f"{INSTAPAPER_READ_URL}{article['id']}"

    save_to_csv(articles, str(csv_file), add_read_url=True)
    ...
    writer = csv.DictWriter(
        output,
        fieldnames=["id", "read_url", "title", "url"],
        quoting=csv.QUOTE_ALL,
    )
    writer.writeheader()
    writer.writerows(articles)
...
def test_save_to_sqlite(sample_articles, output_dir):
    ...
    save_to_sqlite(sample_articles(), str(db_file), add_read_url=False)
...
def test_save_to_sqlite_with_read_url(sample_articles, output_dir):
    """Test saving articles to a SQLite database with the read URL."""
    db_file = output_dir / "bookmarks_with_read_url.db"
    articles = sample_articles()
    for article in articles:
        article["read_url"] = f"{INSTAPAPER_READ_URL}{article['id']}"

    save_to_sqlite(articles, str(db_file), add_read_url=True)

    assert db_file.exists()
    conn = sqlite3.connect(db_file)
    cursor = conn.cursor()
    cursor.execute("SELECT id, read_url, title, url FROM articles ORDER BY id ASC")
    rows = cursor.fetchall()
    conn.close()
...
@pytest.mark.parametrize("add_read_url", [True, False])
@pytest.mark.parametrize(
    "format, filename",
    [("csv", "test.csv"), ("json", "test.json"), ("sqlite", "test.db")],
)
def test_save_articles_dispatcher(
    sample_articles, output_dir, format, filename, add_read_url
):
    """Test the main save_articles dispatcher function."""
    output_file = output_dir / filename
    save_articles(
        sample_articles(), format, str(output_file), add_read_url=add_read_url
    )
